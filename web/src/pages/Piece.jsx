...
import { getPieceById, getAvailablePieces, deletePiece } from "../connection/pieces";
...
export default function PieceDetailPage() {
  ...
  const waMessage = piece
    ...
    : "";
  const waLink = `https://wa.me/${import.meta.env.VITE_WHATSAPP_NUMBER}?text=${waMessage}`;
  const messengerLink = `https://m.me/${import.meta.env.VITE_MESSENGER_ID}`;

  const handleDelete = async () => {
    if (!confirm("¿Estás seguro de que deseas eliminar esta pieza? Esta acción no se puede deshacer.")) return;
    try {
      setDeleting(true);
      await deletePiece(piece.id);
      setModal({ type: "message", message: "Pieza eliminada correctamente." });
      setTimeout(() => navigate("/"), 1500);
    } catch (err) {
      setModal({ type: "error", message: err.response?.data?.message ?? "Error al eliminar." });
      setDeleting(false);
    }
  };

  const handleUpdated = async (updatedPiece) => {
    // Recargar pieza fresca del backend
    try {
      const res = await getPieceById(id);
      if (res.data.success) {
        setPiece(res.data.piece);
      }
    } catch {
      // Fallback: usar la data devuelta
      if (updatedPiece) setPiece(updatedPiece);
    }
    setModal({ type: "message", message: "Pieza actualizada correctamente." });
  };
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start animate-[fadeIn_0.4s_ease]">
          ...
          <div className="space-y-5">
            ...
            {/* Botones admin — solo si está autenticado */}
            {isAuthenticated && (
              <div className="flex gap-3 pt-3 border-t border-border mt-2">
                <button
                  ...
                  className="
                    ...
                    border-2 border-gold text-gold text-sm font-semibold
                    hover:bg-gold hover:text-white
                    active:scale-[0.98]
                    transition-all duration-200 cursor-pointer bg-transparent
                  "
                ...
                <button
                  ...
                  className="
                    inline-flex items-center gap-2 px-4 py-2.5 rounded-lg
                    border-2 border-red-400 text-red-400 text-sm font-semibold
